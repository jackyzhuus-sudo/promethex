...
// VolumeAllTimeLeaderboardHandler Volume All Time Leaderboard消费者处理器
type VolumeAllTimeLeaderboardHandler struct {
...
func NewVolumeAllTimeLeaderboardHandler(
	log log.Logger,
	userBiz *userBiz.UserHandler,
	assetBiz *assetBiz.AssetHandler,
	marketBiz *marketBiz.MarketHandler,
	confCustom *conf.Custom,
	repo base.UsercenterInfra,
) *VolumeAllTimeLeaderboardHandler {
	handler := &VolumeAllTimeLeaderboardHandler{
		log:        log,
		userBiz:    userBiz,
		assetBiz:   assetBiz,
		marketBiz:  marketBiz,
...
func (h *VolumeAllTimeLeaderboardHandler) GetConfig() StreamConsumerConfig {
	return StreamConsumerConfig{
		Name:         "volume_all_time_leaderboard_consumer",
		GroupName:    "volume-all-time-leaderboard-group",
		ConsumerName: "volume-all-time-leaderboard-consumer",
		StreamKeys: []string{
...
func (h *VolumeAllTimeLeaderboardHandler) ProcessMessage(ctx common.Ctx, streamKey string, msg redis.XMessage) error {
	switch streamKey {
...
func (h *VolumeAllTimeLeaderboardHandler) processUserTradeMessage(ctx common.Ctx, msg redis.XMessage) error {
	...
	ctx.Log.Infof("Updated volume all time leaderboard: key=%s, uid=%s, score=%s", leaderboardKey, userTradeStreamMsg.UID, tradeVolume.String())
	return nil
}
