...
// PnlDailyLeaderboardHandler Pnl Daily Leaderboard消费者处理器
type PnlDailyLeaderboardHandler struct {
...
func NewPnlDailyLeaderboardHandler(
	log log.Logger,
	userBiz *userBiz.UserHandler,
	assetBiz *assetBiz.AssetHandler,
	marketBiz *marketBiz.MarketHandler,
	confCustom *conf.Custom,
	repo base.UsercenterInfra,
) *PnlDailyLeaderboardHandler {
	handler := &PnlDailyLeaderboardHandler{
		log:        log,
		userBiz:    userBiz,
		assetBiz:   assetBiz,
		marketBiz:  marketBiz,
...
func (h *PnlDailyLeaderboardHandler) GetConfig() StreamConsumerConfig {
	return StreamConsumerConfig{
		Name:         "pnl_daily_leaderboard_consumer",
		GroupName:    "pnl-daily-leaderboard-group",
		ConsumerName: "pnl-daily-leaderboard-consumer",
		StreamKeys: []string{
...
func (h *PnlDailyLeaderboardHandler) ProcessMessage(ctx common.Ctx, streamKey string, msg redis.XMessage) error {
	switch streamKey {
...
func (h *PnlDailyLeaderboardHandler) processUserTradeMessage(ctx common.Ctx, msg redis.XMessage) error {
	...
	ctx.Log.Infof("Processing pnl daily leaderboard: UID=%s, userTradeStreamMsg: %+v",
	...
	leaderboardKey := fmt.Sprintf(assetBiz.PnlDailyLeaderboard, userTradeStreamMsg.BaseTokenAddress, dayStr)
	err = h.assetBiz.UpdateLeaderboard(ctx, leaderboardKey, userTradeStreamMsg.UID, realizedPnl.InexactFloat64())
	if err != nil {
		ctx.Log.Errorf("failed to update pnl daily leaderboard: %w", err)
		return err
	}

	ctx.Log.Infof("Updated pnl daily leaderboard: key=%s, uid=%s, score=%s", leaderboardKey, userTradeStreamMsg.UID, realizedPnl.String())
