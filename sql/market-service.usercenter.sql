-- 目标库：usercenter
-- 用途：用户中心服务的用户表与索引
-- 注意：由 init 脚本在 \connect usercenter 后加载
-- 创建用户表
CREATE TABLE
    IF NOT EXISTS t_user (
        ...
        invite_at BIGINT NOT NULL DEFAULT 0,
        earned_invite_points BIGINT NOT NULL DEFAULT 0,
        provide_invite_points BIGINT NOT NULL DEFAULT 0,
        privy_user_info jsonb not null default '{}',
...
-- 创建用户社区信息表
CREATE TABLE
    IF NOT EXISTS t_user_community_info (
        id BIGSERIAL PRIMARY KEY,
        created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
        uid VARCHAR(24) NOT NULL,
...
ALTER TABLE t_user
DROP CONSTRAINT user_idx_email;
...
ADD COLUMN base_token_type SMALLINT NOT NULL DEFAULT 1;